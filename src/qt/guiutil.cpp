...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a XTraw address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // Convert xtraw:// to xtraw:
    //
    //    Cannot handle this later, because xtraw:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open xtraw.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "XTRAW.lnk";
}
...
{
    return GetAutostartDir() / "xtraw.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a xtraw.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=XTRAW\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the xtraw app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add xtraw app to startup item list
