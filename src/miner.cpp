...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("XTrawMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("XTrawMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("XTrawMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xtraw-miner");
...
///////|| !masternodeSync.IsSynced()
        ...
        LogPrintf("Running XTrawMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
